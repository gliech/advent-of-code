...
with curse(hide_cursor=True) as scr:
    ...
    debug_container.box()
    main_container.addstr(scr_h-dbg_h-3,scr_w-17,' Distance:   0 ')
    ...
    def bogo_move():
        ...
        sleep(0.01)
        if last_status == 0:
            robot_direction = right_turn[robot_direction]
        else:
            robot_direction = right_turn.index(robot_direction)
        return robot_direction+1
    ...
    last_status = 1
    distance = 0
    known_positions = {(2500,2500):0}
    robot = agc(aoc_data, bogo_move, debug_win)
    ...
    for status in iter(robot):
        ...
        if status == 0:
            ...
            main_container.addstr(scr_h-dbg_h-3,scr_w-7,f'{distance:4}')
